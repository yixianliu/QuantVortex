...
function getHotNumbers(historyData, count = 5) {
  const frequency = {};
  ...
  const sorted = Object.entries(frequency).sort((a, b) => b[1] - a[1]).slice(0, count);
...
function getColdNumbers(historyData, count = 5) {
  var _a, _b;
  const allNumbers = [];
  if ((_a = historyData[0]) == null ? void 0 : _a.group1) {
    const maxGroup1 = historyData[0].group1.length === 6 ? 33 : 35;
    for (let i = 1; i <= maxGroup1; i++)
      allNumbers.push(i);
  ...
  if (((_b = historyData[0]) == null ? void 0 : _b.num1) !== void 0) {
    for (let i = 0; i <= 9; i++)
      allNumbers.push(i);
  }
  const frequency = {};
  ...
  const cold = allNumbers.filter((num) => !frequency[num] || frequency[num] < 3).slice(0, count);
  return cold.map((num) => ({
    num,
...
function analyzeNumberDistribution(historyData) {
  const distribution = {
    odd: 0,
    even: 0,
    small: 0,
...
function generateDetailedReport(lotteryType, historyData) {
  ...
  return {
    lotteryType,
    generateTime: formatDate(/* @__PURE__ */ new Date()),
    ...
    analysis: {
      ...
      coldNumbers,
      distribution,
      strategy: generateStrategy(lotteryType)
    }
  };
}
function generateOptimalReport(lotteryType, historyData) {
  const hotNumbers = getHotNumbers(historyData, 10);
  const coldNumbers = getColdNumbers(historyData, 10);
  const distribution = analyzeNumberDistribution(historyData);
  return {
...
function generateRecommendedNumbers(lotteryType, hotNumbers, coldNumbers) {
  ...
  if (lotteryType === "dataGroupA") {
    const candidates = [.../* @__PURE__ */ new Set([...hotNums.slice(0, 4), ...coldNums.slice(0, 2)])];
    ...
    const candidates = [.../* @__PURE__ */ new Set([...hotNums.slice(0, 3), ...coldNums.slice(0, 2)])];
...
exports.analyzeNumberDistribution = analyzeNumberDistribution;
exports.generateDetailedReport = generateDetailedReport;
exports.generateOptimalReport = generateOptimalReport;
exports.getColdNumbers = getColdNumbers;
exports.getHotNumbers = getHotNumbers;
