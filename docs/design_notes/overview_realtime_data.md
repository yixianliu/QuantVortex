...
- 新增 `SinaFeed` 后，系统默认即走**真实日线/周线**：KP预测、指标、市场全景、回测验证全部基于真实行情运行。
...
4. **`examples/sina_demo.py`**（新）— 拉 rb/cu/au/IF/m/T 真实日线，跑指标+KP预测，输出真实数值并保存 `output/sina_*_forecast.png`（图表标题/图例已英文化，无 CJK 豆腐警告）。
