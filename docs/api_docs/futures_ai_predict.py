...
设计目标：
    ...
    3. 当无网络 / 未配置 API Key / 未安装 requests 时，自动降级到本地
       统计预测引擎（同目录 `local_predictor.Predictor`），保证模块在任何
       环境都能跑通、可被回测与 UI 复用；
...
# ----------------------------------------------------------------------------
# 本地统计引擎降级（复用同目录 local_predictor.Predictor）
# ----------------------------------------------------------------------------
def _local_analyze(symbol: str, bars: List[Dict[str, float]],
                   horizon: int, lookback: int) -> AIAnalysisResult:
    """云端 AI 不可用时的降级分析：用轻量统计外推产出同构的结果对象。

    参数：
        symbol:   合约代码，仅用于回填结果。
        bars:     K 线列表，每项需含 datetime/high/low/close。
        horizon:  预测步数。
        lookback: 回看窗口长度。

    返回：
        AIAnalysisResult，source 标记为 "local_model" 以便调用方区分来源。
    """
    # 延迟导入：仅在真正降级时才加载，避免给云端路径增加无谓依赖。
    # 本文件可能被当作模块 import，此时脚本目录不在 sys.path，需手动补上。
